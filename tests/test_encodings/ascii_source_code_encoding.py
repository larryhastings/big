...
for (encoding, use_encoding, bom, city, invalid) in (
    ('gb18030', 'gb18030', b"\x84\x31\x95\x33", "Beijing", False),
    ('utf-1', 'utf-8', b"\xF7\x64\x4C", "Mountain View", True),
    ):

    bytes = script.format(encoding=encoding, city=city).encode(use_encoding)
    invalid_prefix = "invalid_" if invalid else ""
